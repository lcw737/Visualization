...
df['count'] = 1
'''
origin 컬럼은 제조국가를 표현하는 컬럼으로 1(USA), 2(EU), 3(JPN)을 의미.
이 컬럼을 그룹화 한 후 합계(sum) 연산을 하면 각 국가별 카운트를
계산할 수 있다. 
'''
...
autopct : 퍼센트(%) 표시. 여기서는 1.2f 이므로 소수이하 둘째자리
    까지 표현
startangle : 파이 조각을 나누는 시작점으로 각도를 표시 
'''
