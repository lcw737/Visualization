...
# 숫자형 인덱스를 '이름' 컬럼으러 변경한 후 원본에 적용
print(f"{'이름을 index로 지정 후 출력':-^30}")
df.set_index('이름', inplace=True)
print(df)

print(f"{'0,3요소 수정후':-^30}")
# df.iloc[0][3] = 77 # 경고 발생
# 이와 같이 컴마를 통해 하나의 요소를 직접적으로 지정하는 방식을 사용한다.
df.iloc[0,3] = 77
print(df)

# 문자형 인덱스로 접근 후 요소 수정
print(f"{'서준, 체육 요소 수정후':-^30}")
# df.loc['서준']['체육'] = 66 # 동일한 경고 발생
df.loc['서준', '체육'] = 66
print(df)
...
# 컬럼에서는 '국어'만 남게됨
print(new_df2)
# 인덱스가 2개이므로 요소를 지정할때도 아래와 같이 작성해야한다.
