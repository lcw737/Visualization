...
# 인덱스를 재 지정하는 경우 새롭게 생성된 인덱스에는 NaN이 채워진다.
new_df2 = df.reindex(new_index1)
print(f"{'인덱스 재지정 후 출력':-^30}")
'''
NaN : Not a Number의 이니셜로 직역하면 숫자가 아니다라는 뜻을
    가지고 있다. 유요한 값이 존재하지 않는 누라데이터를 의미한다.
'''
print(new_df2)
...
''' 행 인덱스를 정수형 인덱스로 초기화한다. 이때 기존 행 인덱스는
컬럼으로 이동한다. '''
...
new_df6 = df.reset_index()
print(f"{'인덱스 초기화 후 출력':-^30}")
print(new_df6)

# 행 인덱스를 기준으로 내림차순 정렬
sort_df = new_df3.sort_index(ascending=False)
print(f"{'인덱스를 내림차순 정렬':-^30}")
print(sort_df)

# 특정 컬럼을 지정하고 싶다면 by욥션을 사용
