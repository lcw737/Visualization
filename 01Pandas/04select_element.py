...
'''
데이터프레임에서 특정 요소 하나를 선택할때는 Java의 배열과 유사하게
...
숫자형 인덱스가 자동으로 부여된다. 따라서 '유비'를 통해서는
데이터에 접근할 수 없다.  
'''
# a = df.loc['유비', '수학'] # 오류발생
...
print(a)
print(f"{'0, 2 출력':-^30}")
# 문자형 인덱스를 지정하더라도 숫자형 인덱스로 접근할 수 있다.
b = df.iloc[0, 2] # 유비의 수학점수 접근
print(b)

# 2개 이상의 요소를 선택할때는 컴마를 사용
...
# 범위를 통해 2개 이상의 행과 열을 선택
print(f"{'범위 지정 출력':-^30}")
i = df.loc['유비':'관우', '영어':'체육']
print(i)
# 행은 0~1까지, 열은 1~마지막까지를 선택한다.
j = df.iloc[0:2, 1:]
print(j)