...
'''
데이터프레임(DataFrame)
    :1차원 배열 형태인 시리즈가 여러개 모여있는 형태로 2차원배열과
    동일하다. R의 데이터프레임을 착안하여 만들어지게 되었다.
    행(Row)과 열(Column)로 구성된다. 
'''
'''
딕셔너리는 Key는 컬럼이 되고, Value는 행으로 변환된다.
즉 3행 5열인 데이터 프레임이 생성된다.
'''
...
'''변경된 값을 원본 데이터프레임에 즉시 적용하고 싶다면, inplace 옵션을
추가한다. 해당 옵션을 생략하면 False가 디폴트값이므로 새로운 복사본
객체를 반환하게 된다. '''
df.rename(columns={'연력':'No', '남녀':'Gender', '거주':'City'},
          inplace=True)
df.rename(index={'학생1':'Student1', '학생2':'Studnet2'},
          inplace=True)
print('변경2\n', df)
...
drop() 함수를 통해 행 또는 열을 삭제한다.
    축옵션
        ...
        열 삭제 : axis=1 
    여러개를 동시에 삭제하는 경우에는 리스트형태로 입력한다.
'''
# 축 옵션이 없으므로 첫번째 행이 삭제됨
df.drop('Student1', inplace=True)
print('삭제후1\n', df)

# Gender라는 행이 없으므로 예외가 발생된다.
# df.drop('Gender')

# 열(컬럼)에 대한 삭제이므로 축 옵션을 1로 지정한다.
# 첫번째 실행에는 inplace 옵션이 없으므로 원본 객체에 적용되지 않는다.
df.drop('Gender', axis=1)
# 두번째 실행에서 원본 객체에 적용된다.
