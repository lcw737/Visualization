...
# 행 선택하기
'''
    loc() : 인덱스명을 기준으로 행을 선택. 선택한 모든 범위가 
        포함된다.
    iloc() : 정수형 위치 인덱스를 통해 행을 선택. 마지막 부분은
        제외된다. (파이썬의 기본 규칙과 동일)
'''
print(f"{'유비':-^30}")
# 첫번째 행을 선택해서 출력
...
범위가 아니므로 주의해야한다. 아래 2개의 문장은 동일한 결과를
출력한다. 
'''
print(f"{'유비,장비':-^30}")
label2 = df.loc[['유비', '장비']]
...
'''
1개의 열을 선택했지만, 원소가 리스트로 주어졌으므로 타입은
데이터프레임이 된다. 
'''
print(f"{'수학 열':-^30}")
math2 = df[['수학']]
