...
'''
회귀(Regression) : 통계학에서 변수들 간의 관계를 분석하고 예측하는데 사용되는 통계적
    방법을 의미.
    
regplot() : 회귀선이 있는 산점도를 표현. 서로 다른 2개의 연속 변수 사이의
    산점도를 그리고 선형회귀 분석을 위한 회귀선을 출력.
    회귀의 목적은 데이터에서 패턴을 학습해 새로운 데이터에 대한 예측을
    수행하는 것이다.
'''
# x축은 나이, y축은 운임요금을 표현.
